-- Prep command center: contest_preps, pose_checks, peak_week_plans, peak_week_days.
-- RLS: coach via clients.coach_id, client via clients.user_id (see docs/DB_OWNERSHIP_RULES.md).
...
CREATE UNIQUE INDEX IF NOT EXISTS contest_preps_active_unique
  ON public.contest_preps(client_id)
...
-- contest_preps: client sees own; coach sees clients where coach_id = auth.uid()
DROP POLICY IF EXISTS contest_preps_select_client ON public.contest_preps;
CREATE POLICY contest_preps_select_client ON public.contest_preps
...
CREATE POLICY contest_preps_select_coach ON public.contest_preps
...
CREATE POLICY contest_preps_insert_client ON public.contest_preps
...
CREATE POLICY contest_preps_insert_coach ON public.contest_preps
...
CREATE POLICY contest_preps_update_client ON public.contest_preps
...
CREATE POLICY contest_preps_update_coach ON public.contest_preps
...
CREATE POLICY contest_preps_delete_client ON public.contest_preps
...
CREATE POLICY contest_preps_delete_coach ON public.contest_preps
  FOR DELETE USING (client_id IN (SELECT id FROM public.clients WHERE coach_id = auth.uid()));

-- pose_checks: same pattern
DROP POLICY IF EXISTS pose_checks_select_client ON public.pose_checks;
CREATE POLICY pose_checks_select_client ON public.pose_checks
  FOR SELECT USING (client_id IN (SELECT id FROM public.clients WHERE user_id = auth.uid()));

DROP POLICY IF EXISTS pose_checks_select_coach ON public.pose_checks;
CREATE POLICY pose_checks_select_coach ON public.pose_checks
  FOR SELECT USING (client_id IN (SELECT id FROM public.clients WHERE coach_id = auth.uid()));

DROP POLICY IF EXISTS pose_checks_insert_client ON public.pose_checks;
CREATE POLICY pose_checks_insert_client ON public.pose_checks
  FOR INSERT WITH CHECK (client_id IN (SELECT id FROM public.clients WHERE user_id = auth.uid()));

DROP POLICY IF EXISTS pose_checks_insert_coach ON public.pose_checks;
CREATE POLICY pose_checks_insert_coach ON public.pose_checks
  FOR INSERT WITH CHECK (client_id IN (SELECT id FROM public.clients WHERE coach_id = auth.uid()));

DROP POLICY IF EXISTS pose_checks_update_client ON public.pose_checks;
CREATE POLICY pose_checks_update_client ON public.pose_checks
  FOR UPDATE USING (client_id IN (SELECT id FROM public.clients WHERE user_id = auth.uid()));

DROP POLICY IF EXISTS pose_checks_update_coach ON public.pose_checks;
CREATE POLICY pose_checks_update_coach ON public.pose_checks
  FOR UPDATE USING (client_id IN (SELECT id FROM public.clients WHERE coach_id = auth.uid()));

DROP POLICY IF EXISTS pose_checks_delete_client ON public.pose_checks;
CREATE POLICY pose_checks_delete_client ON public.pose_checks
  FOR DELETE USING (client_id IN (SELECT id FROM public.clients WHERE user_id = auth.uid()));

DROP POLICY IF EXISTS pose_checks_delete_coach ON public.pose_checks;
CREATE POLICY pose_checks_delete_coach ON public.pose_checks
  FOR DELETE USING (client_id IN (SELECT id FROM public.clients WHERE coach_id = auth.uid()));

-- peak_week_plans: same (has client_id)
DROP POLICY IF EXISTS peak_week_plans_select_client ON public.peak_week_plans;
CREATE POLICY peak_week_plans_select_client ON public.peak_week_plans
  FOR SELECT USING (client_id IN (SELECT id FROM public.clients WHERE user_id = auth.uid()));

DROP POLICY IF EXISTS peak_week_plans_select_coach ON public.peak_week_plans;
CREATE POLICY peak_week_plans_select_coach ON public.peak_week_plans
  FOR SELECT USING (client_id IN (SELECT id FROM public.clients WHERE coach_id = auth.uid()));

DROP POLICY IF EXISTS peak_week_plans_insert_client ON public.peak_week_plans;
CREATE POLICY peak_week_plans_insert_client ON public.peak_week_plans
  FOR INSERT WITH CHECK (client_id IN (SELECT id FROM public.clients WHERE user_id = auth.uid()));

DROP POLICY IF EXISTS peak_week_plans_insert_coach ON public.peak_week_plans;
CREATE POLICY peak_week_plans_insert_coach ON public.peak_week_plans
  FOR INSERT WITH CHECK (client_id IN (SELECT id FROM public.clients WHERE coach_id = auth.uid()));

DROP POLICY IF EXISTS peak_week_plans_update_client ON public.peak_week_plans;
CREATE POLICY peak_week_plans_update_client ON public.peak_week_plans
  FOR UPDATE USING (client_id IN (SELECT id FROM public.clients WHERE user_id = auth.uid()));

DROP POLICY IF EXISTS peak_week_plans_update_coach ON public.peak_week_plans;
CREATE POLICY peak_week_plans_update_coach ON public.peak_week_plans
  FOR UPDATE USING (client_id IN (SELECT id FROM public.clients WHERE coach_id = auth.uid()));

DROP POLICY IF EXISTS peak_week_plans_delete_client ON public.peak_week_plans;
CREATE POLICY peak_week_plans_delete_client ON public.peak_week_plans
  FOR DELETE USING (client_id IN (SELECT id FROM public.clients WHERE user_id = auth.uid()));

DROP POLICY IF EXISTS peak_week_plans_delete_coach ON public.peak_week_plans;
CREATE POLICY peak_week_plans_delete_coach ON public.peak_week_plans
  FOR DELETE USING (client_id IN (SELECT id FROM public.clients WHERE coach_id = auth.uid()));

-- peak_week_days: no client_id; derive via plan -> peak_week_plans.client_id
DROP POLICY IF EXISTS peak_week_days_select_client ON public.peak_week_days;
CREATE POLICY peak_week_days_select_client ON public.peak_week_days
  FOR SELECT USING (
...
CREATE POLICY peak_week_days_select_coach ON public.peak_week_days
  FOR SELECT USING (
...
CREATE POLICY peak_week_days_insert_client ON public.peak_week_days
  FOR INSERT WITH CHECK (
    plan_id IN (SELECT id FROM public.peak_week_plans WHERE client_id IN (SELECT id FROM public.clients WHERE user_id = auth.uid()))
  );

DROP POLICY IF EXISTS peak_week_days_insert_coach ON public.peak_week_days;
CREATE POLICY peak_week_days_insert_coach ON public.peak_week_days
  FOR INSERT WITH CHECK (
    plan_id IN (SELECT id FROM public.peak_week_plans WHERE client_id IN (SELECT id FROM public.clients WHERE coach_id = auth.uid()))
  );

DROP POLICY IF EXISTS peak_week_days_update_client ON public.peak_week_days;
CREATE POLICY peak_week_days_update_client ON public.peak_week_days
  FOR UPDATE USING (
    plan_id IN (SELECT id FROM public.peak_week_plans WHERE client_id IN (SELECT id FROM public.clients WHERE user_id = auth.uid()))
  );

DROP POLICY IF EXISTS peak_week_days_update_coach ON public.peak_week_days;
CREATE POLICY peak_week_days_update_coach ON public.peak_week_days
  FOR UPDATE USING (
    plan_id IN (SELECT id FROM public.peak_week_plans WHERE client_id IN (SELECT id FROM public.clients WHERE coach_id = auth.uid()))
  );

DROP POLICY IF EXISTS peak_week_days_delete_client ON public.peak_week_days;
CREATE POLICY peak_week_days_delete_client ON public.peak_week_days
  FOR DELETE USING (
    plan_id IN (SELECT id FROM public.peak_week_plans WHERE client_id IN (SELECT id FROM public.clients WHERE user_id = auth.uid()))
  );

DROP POLICY IF EXISTS peak_week_days_delete_coach ON public.peak_week_days;
CREATE POLICY peak_week_days_delete_coach ON public.peak_week_days
  FOR DELETE USING (
    plan_id IN (SELECT id FROM public.peak_week_plans WHERE client_id IN (SELECT id FROM public.clients WHERE coach_id = auth.uid()))
  );
