-- Exact FK coverage from CSV-2 remaining unindexed_foreign_keys rows.
-- Builds indexes from pg_constraint.conkey (actual FK column order), not name guesses.
-- Then removes legacy generated lint-fix index names that created advisor churn.
...
BEGIN
  ...
  LOOP
    SELECT c.conrelid, c.conkey, n.nspname, cls.relname
    INTO v_conrelid, v_conkey, v_schema_name, v_table_name
    ...
    JOIN pg_attribute a ON a.attrelid = v_conrelid AND a.attnum = k.attnum;
    ...
    v_index_name := format('idx_lintcov_%s_%s', left(v_table_name, 28), left(md5(v_constraint_name), 12));

    EXECUTE format(
      'CREATE INDEX IF NOT EXISTS %I ON %I.%I (%s)',
      v_index_name, v_schema_name, v_table_name, v_cols_sql
    );
  END LOOP;
END $$;
...
BEGIN
  FOR r IN
    ...
    WHERE n.nspname = 'public'
      AND (i.relname LIKE 'idx_fkcov_%' OR i.relname LIKE 'idx_covfix_%')
