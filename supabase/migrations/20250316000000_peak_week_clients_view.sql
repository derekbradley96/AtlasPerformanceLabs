-- Peak Week Command Center: clients with active prep and show_date within 14 days.
-- Sources: contest_preps, clients, checkins, pose_checks, client_state, client_phases.
-- Filter: is_active = true and days_out <= 14 (show_date - current_date).
...
SELECT
  ...
  -- clients.name only on some DBs (full_name may not exist)
...
COMMENT ON VIEW public.v_peak_week_clients IS 'Peak Week Command Center: clients with active contest prep and show in ≤14 days. Query: WHERE coach_id = auth.uid() ORDER BY days_out ASC.';
