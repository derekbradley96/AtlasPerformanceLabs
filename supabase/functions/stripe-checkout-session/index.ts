...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!coach) return new Response(JSON.stringify({ error: "Coach not found" }), { status: 404, headers: { ...getCorsHeaders(req), "Content-Type": "application/json" } });
    if (!coach.stripe_account_id) return new Response(JSON.stringify({ error: "Coach has no Stripe Connect account; connect in Earnings first" }), { status: 400, headers: { ...getCorsHeaders(req), "Content-Type": "application/json" } });
    if (!coach.charges_enabled) return new Response(JSON.stringify({ error: "Coach Stripe account not ready for payments" }), { status: 400, headers: { ...getCorsHeaders(req), "Content-Type": "application/json" } });
    ...
    if (!service?.stripe_price_id) return new Response(JSON.stringify({ error: "Service or Stripe price not found" }), { status: 404, headers: { ...getCorsHeaders(req), "Content-Type": "application/json" } });

    let leadId = lead_id;
    const name = (lead_name ?? "").trim() || "Customer";
    const email = (lead_email ?? "").trim();
    if (!email) return new Response(JSON.stringify({ error: "lead_email required" }), { status: 400, headers: { ...getCorsHeaders(req), "Content-Type": "application/json" } });
    ...
    if (!leadId) {
      ...
      if (leadErr) return new Response(JSON.stringify({ error: leadErr.message }), { status: 500, headers: { ...getCorsHeaders(req), "Content-Type": "application/json" } });
      leadId = newLead.id;
    }
    ...
    return new Response(JSON.stringify({ url, session_id: session.id, lead_id: leadId }), { headers: { ...getCorsHeaders(req), "Content-Type": "application/json" } });
  } catch (e) {
