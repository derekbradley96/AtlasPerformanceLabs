// Create Stripe Checkout Session (subscription) for a lead + service; return checkout URL
...
// Commission by plan: Basic 10%, Pro 3%, Elite 0%. Normalize for DB casing. Missing plan_tier defaults to 10% (no throw).
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { coach_id, service_id, lead_id, lead_name, lead_email } = body;
    const userId = body.user_id ?? coach_id;
    if (!userId || !service_id) return new Response(JSON.stringify({ error: "coach_id and service_id required" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });

    const supabase = createClient(Deno.env.get("SUPABASE_URL")!, Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!);
    const { data: coach } = await supabase.from(TABLE.coaches).select("id, stripe_account_id, charges_enabled, plan_tier").eq("user_id", userId).single();
    ...
    console.log("[stripe-checkout-session] coach_id=" + coach.id + " plan_tier=" + (coach.plan_tier ?? "null") + " feePercent=" + feePercent);
    ...
    const successUrl = `${origin.replace(/\/$/, "")}/lead-checkout/success?session_id={CHECKOUT_SESSION_ID}`;
    const cancelUrl = `${origin.replace(/\/$/, "")}/lead-checkout/cancel`;
