...
import { lightHaptic, timelineDateLabel, timelineIconForBadge, TIMELINE_FILTERS } from '@/pages/client-detail/clientDetailUtils';

/**
 * Full performance timeline (raw `timelineEvents`) with filters — separate from merged OS column.
 */
export default function ClientDetailTimelineHistorySheet({
  open,
  onOpenChange,
  timelineFilter,
          ...
          <div className="flex flex-wrap gap-2" style={{ marginBottom: spacing[12] }}>
            {TIMELINE_FILTERS.map((f) => (
              <button
                key={f.key}
                type="button"
                onClick={() => { lightHaptic(); onTimelineFilterChange(f.key); }}
