 ...
 * @param {unknown} [viewerWeightUnit] - Logged-in viewer's bodyweight display unit (kg / st_lb / lb); defaults to kg
 * @returns {import('../types').ReviewItem}
 */
export function checkinToReviewItem(thisWeek, lastWeek, client, risk, viewerWeightUnit = 'kg') {
  ...
  return {
    ...
    left: {
      title: 'This week',
      metrics: [
        { label: 'Weight (avg)', value: tw.weight_kg != null ? formatWeightForViewer(tw.weight_kg, wu) : null, delta: weightDelta, deltaWarning: weightNote, trend: weightTrend },
    ...
    right: lw ? {
      title: 'Last week',
      metrics: [
        { label: 'Weight (avg)', value: lw.weight_kg != null ? formatWeightForViewer(lw.weight_kg, wu) : null },
    ...
    suggestedAction,
    diffRows: lw ? [
      { label: 'Weight', curr: tw.weight_kg, prev: lw.weight_kg, format: (v) => (v != null ? formatWeightForViewer(v, wu) : '—'), delta: tw.weight_kg != null && lw.weight_kg != null && lw.weight_kg > 0 ? ((tw.weight_kg - lw.weight_kg) / lw.weight_kg) * 100 : null },
