...
export default function CoachRoutes({
  ...
  return (
    <>
      ...
      <Route path="review-center/checkins/:checkinId" element={<RequireRole allow={[Roles.COACH, Roles.ADMIN]} accessDeniedMessage="Review Center is for coaches only."><Suspense fallback={<LazyRouteFallback />}><CheckInReviewPage /></Suspense></RequireRole>} />
      ...
      {/* Shared across coach, client, and personal — this registration wins (first match), so all three roles must be allowed here. */}
      <Route path="prep-comparison" element={<RequireRole allow={[Roles.COACH, Roles.CLIENT, Roles.PERSONAL, Roles.ADMIN]} accessDeniedMessage="Photo comparison is for coaches, clients, and personal accounts."><PrepComparisonPage /></RequireRole>} />
